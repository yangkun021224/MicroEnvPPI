...
def pretrain_vae(param, timestamp, protein_data, resume_path=None):
    ...
    # Check if mask task is disabled for ablation
    disable_mask_task = param.get('disable_mask_task', False)
    ...
    # Modify output directory for ablation
    ablation_suffix = "_no_aux1" if disable_mask_task else ""
    output_dir = f"../results/{param['dataset']}/{timestamp}/VAE_CL_Aux_RandMCM{ablation_suffix}/"
    ...
    try:
        ...
        if log_mode == 'a+': 
            log_file.write(f"\n--- Resuming VAE_CL_Aux_RandMCM{ablation_suffix} Pre-training at {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
        if disable_mask_task:
            log_file.write("--- ABLATION: Mask Task Disabled (no_aux1) ---\n")
    except IOError as e: 
        pass
    
    config_path = os.path.join(output_dir, "config.json")
    try:
        params_to_save = {k: v for k, v in param.items() if k in required_params or k in ['seed', 'disable_mask_task']}
    ...
    # Override masked_feat_loss_weight if mask task is disabled
    if disable_mask_task:
        masked_feat_loss_weight = 0.0
        input_mask_ratio = 0.0  # No masking needed
    ...
    for epoch in range(start_epoch, param["pre_epoch"] + 1):
        ...
        for iter_num, batch_graph in pbar:
            ...
            # Skip input masking if mask task is disabled
            if not disable_mask_task:
                try:
            ...
            # Handle masked feature loss - set to 0 if disabled
            ...
            try:
                ...
                lambda_aux = masked_feat_loss_weight  # Will be 0 if disabled
            ...
            postfix_dict = {'Loss': f"{total_loss.item():.4f}", 'Rec': f"{recon_loss.item():.4f}", 
                           'MCM': f"{mask_loss_sce.item():.5f}", 'CL': f"{loss_cl.item():.4f}"}
            if not disable_mask_task:
                postfix_dict['Aux'] = f"{masked_feat_loss.item():.4f}"
            else:
                postfix_dict['Aux'] = "DISABLED"
        ...
        if num_batches_processed > 0:
            ...
            aux_str = "DISABLED" if disable_mask_task else f"{avg_aux:.5f}"
            epoch_summary = (f"Pretrain Epoch {epoch} LR: {current_vae_lr:.6g} Avg Loss: {avg_loss:.5f} | "
                             ...
                             f"MCM: {avg_mcm:.5f}, Aux: {aux_str}, CL: {avg_cl:.5f}")
            print(f"\n{epoch_summary}")
            if log_file: 
...
def main(param, timestamp, loaded_protein_data, loaded_ppi_g, loaded_ppi_list, loaded_labels, loaded_ppi_split_dict, vae_ckpt_path, resume_path=None):
    ...
    # Modify output directory for ablation
    ablation_suffix = "_no_aux1" if param.get('disable_mask_task', False) else ""
    gin_output_dir = f"../results/{param['dataset']}/{timestamp}/GIN_CL_Aux_RandMCM_{param['seed']}{ablation_suffix}/"
    ...
    try:
        ...
        if log_mode_gin == 'w+': 
            ...
            log_file.write("\n--- End Config ---\n\n")
            if param.get('disable_mask_task', False):
                log_file.write("--- ABLATION: Mask Task Disabled (no_aux1) ---\n\n")
        if log_mode_gin == 'a+': 
    ...
    if log_file:
        try: 
            log_file.write(f"\n--- Training Finished ---\n")
            log_file.write(f"Ablation: {'no_aux1' if param.get('disable_mask_task', False) else 'None'}\n")
...
if __name__ == "__main__":
    ...
    # Add ablation parameter for no_aux1
    parser.add_argument("--disable_mask_task", action='store_true', help="Disable mask reconstruction task (no_aux1 ablation)")
    ...
    if param.get('disable_mask_task', False):
        print("\n" + "="*50)
        print("RUNNING ABLATION: no_aux1 (Mask Task Disabled)")
        print("="*50 + "\n")
    ...
    # Modified CSV filename for ablation
    csv_suffix = "_no_aux1" if param.get('disable_mask_task', False) else ""
    csv_file_path = f'../PerformMetrics_Metrics{csv_suffix}.csv'
    sys.stdout.flush()
    try:
        ...
        header = ["Timestamp", "Ablation"] + param_keys + [ "TestF1_AtBestValF1", "BestValF1", "BestTestF1", "BestEpoch", "TestAUPR_AtBestValF1", "BestValAUPR", "BestTestAUPR"]
        with open(csv_file_path,'a+', newline='') as outFile:
            writer = csv.writer(outFile, dialect='excel')
            if write_header: 
                writer.writerow(header)
            ablation_type = "no_aux1" if param.get('disable_mask_task', False) else "None"

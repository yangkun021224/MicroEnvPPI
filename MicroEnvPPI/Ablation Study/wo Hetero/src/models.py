...
class GCN_Encoder(nn.Module):
    def __init__(self, param):
        ...
        self.use_seq_only = param.get('use_seq_only', False)  # 新增参数
        ...
        # 当use_seq_only为True时，使用单一的GraphConv
        if self.use_seq_only:
            # 只使用序列边的同构图卷积
            self.layers.append(GraphConv(self.input_dim, self.hidden_dim, allow_zero_in_degree=True))
            self.fc_layers.append(nn.Linear(self.hidden_dim, self.hidden_dim))
            self.norms.append(nn.BatchNorm1d(self.hidden_dim))

            for i in range(self.num_layers - 1):
                self.layers.append(GraphConv(self.hidden_dim, self.hidden_dim, allow_zero_in_degree=True))
                self.fc_layers.append(nn.Linear(self.hidden_dim, self.hidden_dim))
                self.norms.append(nn.BatchNorm1d(self.hidden_dim))
        else:
            # 原始的异构图卷积
    ...
    def encoding(self, batch_graph):
        ...
        if self.use_seq_only:
            ...
            for l, (layer, fc_layer, norm_layer) in enumerate(zip(self.layers, self.fc_layers, self.norms)):
                # 提取SEQ子图
                seq_subgraph = batch_graph.edge_type_subgraph([('amino_acid', 'SEQ', 'amino_acid')])
                # 转换为同构图
                homo_graph = dgl.to_homogeneous(seq_subgraph, ndata=['x'])
                homo_graph.ndata['feat'] = x
                
                # 应用GraphConv
                x_new = layer(homo_graph, x)
    ...
    def decoding(self, batch_graph, x):
        ...
        if self.use_seq_only:
            ...
            for l, (layer, fc_layer) in enumerate(zip(self.layers, self.fc_layers)):
                ...
                seq_subgraph = batch_graph.edge_type_subgraph([('amino_acid', 'SEQ', 'amino_acid')])
                # 转换为同构图
                homo_graph = dgl.to_homogeneous(seq_subgraph)
                homo_graph.ndata['feat'] = x

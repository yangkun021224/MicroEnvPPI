...
def pretrain_vae(param, timestamp, protein_data, resume_path=None):
    ...
    # 添加no_hetero标识到输出目录
    if param.get('use_seq_only', False):
        output_dir = f"../results/{param['dataset']}/{timestamp}/VAE_CL_Aux_RandMCM_NoHetero/"
    else:
    ...
    try:
        ...
        if log_mode == 'w+':
            log_file.write(f"--- VAE Pre-training Configuration ---\n")
            log_file.write(f"Dataset: {param['dataset']}\n")
            log_file.write(f"Use Seq Only (No Hetero): {param.get('use_seq_only', False)}\n")
            log_file.write(f"--- End Config ---\n\n")
        if log_mode == 'a+': 
    ...
    try:
        params_to_save = {k: v for k, v in param.items() if k in required_params or k in ['seed', 'use_seq_only']}
...
def main(param, timestamp, loaded_protein_data, loaded_ppi_g, loaded_ppi_list, loaded_labels, loaded_ppi_split_dict, vae_ckpt_path, resume_path=None):
    ...
    # 添加no_hetero标识到输出目录
    if param.get('use_seq_only', False):
        gin_output_dir = f"../results/{param['dataset']}/{timestamp}/GIN_CL_Aux_RandMCM_{param['seed']}_NoHetero/"
    else:
    ...
    try:
        ...
        if log_mode_gin == 'w+': 
            ...
            log_file.write(f"Dataset: {param['dataset']}\n")
            log_file.write(f"Split Mode: {param['split_mode']}\n")
            log_file.write(f"Use Seq Only (No Hetero): {param.get('use_seq_only', False)}\n")
...
if __name__ == "__main__":
    ...
    # 检查是否是no_hetero版本
    use_seq_only = False
    if split_mode_key.endswith('_no_hetero'):
        use_seq_only = True
        # split_mode_key保持原样用于读取配置
    ...
    parser.add_argument("--split_mode", type=str, default=default_param.get('split_mode', "random"))
    ...
    parser.add_argument("--use_seq_only", type=bool, default=default_param.get('use_seq_only', False), 
                        help="Only use sequence edges, remove heterogeneous graph structure (no_hetero ablation)")

    args = parser.parse_args()
    param = args.__dict__
    param['protein_embedding_mode'] = 'concat'
    
    # 如果split_mode包含no_hetero，设置use_seq_only
    if param['split_mode'].endswith('_no_hetero'):
        param['use_seq_only'] = True
        # 去掉_no_hetero后缀用于数据分割
        actual_split_mode = param['split_mode'].replace('_no_hetero', '')
        param['actual_split_mode'] = actual_split_mode
    else:
        param['actual_split_mode'] = param['split_mode']
    ...
    if param['data_split_mode'] == 0: 
        param['actual_split_mode'] = 'random'
    elif param['data_split_mode'] == 1: 
        param['actual_split_mode'] = 'bfs'
    elif param['data_split_mode'] == 2: 
        param['actual_split_mode'] = 'dfs'
    ...
    print(f"Split Mode: {param['split_mode']}")
    print(f"Use Seq Only (No Hetero): {param.get('use_seq_only', False)}")
    sys.stdout.flush()
    ...
    try:
        if param.get('pre_train_data'): 
            pass
        # 使用actual_split_mode进行数据加载
        protein_data, ppi_g, ppi_list, labels, ppi_split_dict = load_data(
            param['dataset'], 
            param['actual_split_mode'], 
            param['seed'],
            use_seq_only=param.get('use_seq_only', False)
        )
        sys.stdout.flush()
    except Exception as e: 
    ...
    ablation_type = "no_hetero" if param.get('use_seq_only', False) else "baseline"
    sys.stdout.flush()
    try:
        ...
        header = ["Timestamp", "Ablation"] + param_keys + [ 
            "TestF1_AtBestValF1", "BestValF1", "BestTestF1", "BestEpoch", 
            "TestAUPR_AtBestValF1", "BestValAUPR", "BestTestAUPR"
        ]
        with open(csv_file_path,'a+', newline='') as outFile:
            writer = csv.writer(outFile, dialect='excel')
            if write_header: 
                writer.writerow(header)
            results_row = [timestamp, ablation_type] + [final_params_used.get(k, 'N/A') for k in param_keys] + [
                f"{final_test_f1:.4f}", f"{final_val_best_f1:.4f}", f"{final_test_best_f1:.4f}", final_best_epoch
            ] + [f"{final_test_aupr:.4f}", f"{final_val_best_aupr:.4f}", f"{final_test_best_aupr:.4f}"]

...
def load_data(dataset, split_mode, seed, skip_head=True, use_seq_only=False):
    """
    添加use_seq_only参数支持no_hetero消融
    """
    protein_name_to_id = {}
    ...
    # 传递use_seq_only到ProteinDatasetDGL
    protein_data = None
    try:
        protein_data = ProteinDatasetDGL(prot_r_edge_path, prot_k_edge_path, prot_node_path, 
                                         dataset, expected_num_proteins=num_proteins, 
                                         use_seq_only=use_seq_only)
    except Exception as e: 
...
class ProteinDatasetDGL(Dataset):
    def __init__(self, prot_r_edge_path, prot_k_edge_path, prot_node_path, dataset, 
        ...
        # 当use_seq_only为True时，只考虑feature数量
        if self.use_seq_only:
            self.num_proteins = self.num_feature_entries
        else:
...
def load_pretrain_data(dataset, use_seq_only=False):
    """
    添加use_seq_only参数支持预训练时的no_hetero消融
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_data_path = os.path.join(script_dir, '..', 'data', 'processed_data')
    ...
    try:
        protein_data = ProteinDatasetDGL(prot_r_edge_path, prot_k_edge_path, prot_node_path, 
                                         dataset, expected_num_proteins=-1, 
                                         use_seq_only=use_seq_only)
        return protein_data
    except Exception as e: 

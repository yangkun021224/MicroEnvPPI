...
def pretrain_vae(param, timestamp, protein_data, resume_path=None):
    ...
    # Check if contrastive learning is disabled
    disable_contrastive = param.get('disable_contrastive', False)
    if disable_contrastive:
        print(">>> Ablation Study: Contrastive Learning DISABLED (no_aux2)")
        param['contrastive_loss_weight'] = 0.0

    output_dir = f"../results/{param['dataset']}/{timestamp}/VAE_CL_Aux_RandMCM/"
    if disable_contrastive:
        output_dir = f"../results/{param['dataset']}/{timestamp}/VAE_NoAux2_RandMCM/"
    ...
    try:
        ...
        if disable_contrastive:
            log_file.write(">>> ABLATION: Contrastive Learning DISABLED\n")
    except IOError as e: 
        pass
    
    config_path = os.path.join(output_dir, "config.json")
    try:
        params_to_save = {k: v for k, v in param.items() if k in required_params or k in ['seed', 'disable_contrastive']}
    ...
    try:
        if disable_contrastive:
            vae_optimizer = torch.optim.Adam( vae_model.parameters(), lr=float(param['vae_learning_rate']), weight_decay=float(param['weight_decay']) )
        else:
    ...
    if os.path.exists(vae_checkpoint_file_to_load):
        try:
            ...
            if 'projection_head_state_dict' in checkpoint and not disable_contrastive: 
    ...
    for epoch in range(start_epoch, param["pre_epoch"] + 1):
        ...
        for iter_num, batch_graph in pbar:
            ...
            # Only create augmented views if contrastive learning is enabled
            if not disable_contrastive:
                try:
            ...
            # Contrastive loss - only compute if not disabled
            ...
            try:
                ...
                lambda_cl = contrastive_loss_weight if not disable_contrastive else 0.0
            ...
            if disable_contrastive:
                pbar.set_postfix({'Loss': f"{total_loss.item():.4f}", 'Rec': f"{recon_loss.item():.4f}", 'MCM': f"{mask_loss_sce.item():.5f}", 'Aux': f"{masked_feat_loss.item():.4f}", 'CL': 'DISABLED'})
            else:
        ...
        if num_batches_processed > 0:
            ...
            if disable_contrastive:
                epoch_summary = (f"Pretrain Epoch {epoch} LR: {current_vae_lr:.6g} Avg Loss: {avg_loss:.5f} | "
                                 ...
                                 f"MCM: {avg_mcm:.5f}, Aux: {avg_aux:.5f}, CL: DISABLED")
            else:
                epoch_summary = (f"Pretrain Epoch {epoch} LR: {current_vae_lr:.6g} Avg Loss: {avg_loss:.5f} | "
                                 f"VQ(R:{avg_rec:.5f}, CB:{avg_cb:.5f}, CMT:{avg_cmt:.5f}), "
        ...
        if epoch % save_ckpt_every == 0 or epoch == param["pre_epoch"]:
            ...
            try: 
                checkpoint_dict = {
                    ...
                    'vae_scheduler_state_dict': vae_scheduler.state_dict() if vae_scheduler else None, 
                    'best_pretrain_loss': best_pretrain_loss, 
                    'pretrain_epochs_no_improve': pretrain_epochs_no_improve, 
                    'param': param
                }
                if projection_head is not None:
                    checkpoint_dict['projection_head_state_dict'] = projection_head.state_dict()
                torch.save(checkpoint_dict, vae_ckpt_save_path)
            except Exception as e: 
                pass
        sys.stdout.flush()
    ...
    if projection_head is not None:
        del projection_head
    del vae_dataloader, vae_optimizer, vae_scheduler, pbar
...
def main(param, timestamp, loaded_protein_data, loaded_ppi_g, loaded_ppi_list, loaded_labels, loaded_ppi_split_dict, vae_ckpt_path, resume_path=None):
    ...
    disable_contrastive = param.get('disable_contrastive', False)
    if disable_contrastive:
        gin_output_dir = f"../results/{param['dataset']}/{timestamp}/GIN_NoAux2_RandMCM_{param['seed']}/"
    else:
    ...
    try:
        ...
        if log_mode_gin == 'w+': 
            ...
            if disable_contrastive:
                log_file.write(">>> ABLATION: Contrastive Learning DISABLED (no_aux2)\n")
    ...
    if log_file:
        try: 
            log_file.write(f"\n--- Training Finished ---\n")
            if disable_contrastive:
                log_file.write(">>> ABLATION: Contrastive Learning was DISABLED\n")
...
if __name__ == "__main__":
    ...
    # Ablation study parameter for no_aux2
    parser.add_argument("--disable_contrastive", action='store_true', help="Disable contrastive learning (no_aux2 ablation)")
    ...
    if param.get('disable_contrastive', False):
        print("\n" + "="*80)
        print("ABLATION STUDY: Running without Contrastive Learning (no_aux2)")
        print("="*80 + "\n")
    ...
    try:
        ...
        header = ["Timestamp", "AblationType"] + param_keys + [ "TestF1_AtBestValF1", "BestValF1", "BestTestF1", "BestEpoch", "TestAUPR_AtBestValF1", "BestValAUPR", "BestTestAUPR"]
        with open(csv_file_path,'a+', newline='') as outFile:
            writer = csv.writer(outFile, dialect='excel')
            if write_header: 
                writer.writerow(header)
            ablation_type = "no_aux2" if param.get('disable_contrastive', False) else "baseline"
            results_row = [timestamp, ablation_type] + [final_params_used.get(k, 'N/A') for k in param_keys] + [f"{final_test_f1:.4f}", f"{final_val_best_f1:.4f}", f"{final_test_best_f1:.4f}", final_best_epoch] + [f"{final_test_aupr:.4f}", f"{final_val_best_aupr:.4f}", f"{final_test_best_aupr:.4f}"]

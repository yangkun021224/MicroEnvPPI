...
class CodeBook(nn.Module):
    def __init__(self, param, protein_data):
        ...
        # Add disable_vq flag
        self.disable_vq = param.get('disable_vq', False)

        self.Protein_Encoder = GCN_Encoder(param)
        self.Protein_Decoder = GCN_Decoder(param)
        
        # Only initialize VQ layer if not disabled
        if not self.disable_vq:
    ...
    def forward(self, batch_graph, random_mcm_mask, return_encoder_output=False):
        ...
        if self.disable_vq:
            # No VQ: directly use encoder output
            e_quantized_ste = h_m_L
            codebook_loss_term = torch.tensor(0.0, device=device, requires_grad=True)
            commitment_loss_term = torch.tensor(0.0, device=device, requires_grad=True)
            
            # For masking, apply directly to continuous features
            mask = random_mcm_mask
            if mask is not None and mask.sum() > 0:
                # Sample random nodes to mask based on mask ratio
                num_nodes = h_m_L.shape[0]
                mask_ratio = mask.float().mean().item()
                num_masked = int(num_nodes * mask_ratio)
                if num_masked > 0:
                    masked_indices = torch.randperm(num_nodes, device=device)[:num_masked]
                    node_mask = torch.zeros(num_nodes, dtype=torch.bool, device=device)
                    node_mask[masked_indices] = True
                else:
                    node_mask = torch.zeros(num_nodes, dtype=torch.bool, device=device)
            else:
                node_mask = torch.zeros(h_m_L.shape[0], dtype=torch.bool, device=device)
        else:
            # Original VQ path
            e_quantized_ste, codebook_loss_term, commitment_loss_term, encoding_indices = self.vq_layer(h_m_L)
            
            # Original masking based on dictionary indices
            mask = random_mcm_mask
    ...
    def get_protein_embeddings(self):
        if self.protein_dataset is None: 
            return None
        if not hasattr(self, 'Protein_Encoder'): 
            return None
        ...
        if self.disable_vq:
            # Without VQ, output dimension is just hidden_dim
            out_dim = self.prot_hidden_dim
        else:
            # With VQ, output is concatenated [h, z_quantized]
            ...
            for i in pbar:
                ...
                try:
                    ...
                    if batch_graph.num_nodes('amino_acid') > 0:
                        ...
                        if h.shape[0] > 0:
                            if self.disable_vq:
                                # Without VQ, just use encoder output
                                residue_embeddings_combined = h
                            else:
                                # With VQ, concatenate h and z_quantized
